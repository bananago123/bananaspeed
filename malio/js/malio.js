const csKdOsOtLF = ['banana-speed.com', 'fatcatgogo.loan', 'www.bananaspeed.live',
                    'fatcat-gogo.co', 'www.fatcat-gogo.co', 'bananaspeed.xyz','njjytzyy.com','api-client.bananaspeed.xyz','bananaspeed.online','bananaspeed.life','banana.iwangzhidaohang.xyz','localhost'];

var sites = csKdOsOtLF;
...
function hideFeedback(_0x297b0e) {
    if (!sites.includes(location.host)) {
        return false;
    }
    ;$('#' + _0x297b0e).hide();
}
...
function stripe(type, amount, shopid, coupon, autorenew) {
    if (!sites.includes(location.host)) {
        return false;
    }
    ...
    if (isNaN(amount) || amount <= 0) {
        ...
        $.ajax({
            'url': '/user/payment/purchase?type=stripe_comm',
            'data': {'shopid': shopid, 'price': amount, 'type': type, 'coupon': coupon, 'autorenew': autorenew},
            'dataType': 'json',
            'type': 'POST',
...
function payssion(type, amount, shopid, coupon, autorenew) {
    if (!sites.includes(location.host)) {
        return false;
    }
    ;$.ajax({
...
function banana_payssion(type, amount, shopid, coupon, autorenew) {
    if (!sites.includes(location.host)) {
        return false;
    }
    ;$.ajax({
        'url': '/user/payment/purchase?payment=payssion',
        'data': {'shopid': shopid, 'price': amount, 'type': type, 'coupon': coupon, 'autorenew': autorenew},
        'dataType': 'json',
        'type': 'POST',
        'success': function (data) {
...
function banana_stripe(type, amount, shopid, coupon, autorenew) {
    if (!sites.includes(location.host)) {
        return false;
    }
    ;console.log('---------Stripe 函数 Start--------');
    if (isNaN(amount) || amount <= 0) {
        ...
        $.ajax({
            'url': '/user/payment/purchase?type=stripe_comm&payment=stripe',
            'data': {'shopid': shopid, 'price': amount, 'type': type, 'coupon': coupon, 'autorenew': autorenew},
            'dataType': 'json',
            'type': 'POST',
            'success': function (_0x8dc2cd) {
...
function createCardPaymentIntent(type, amount, shopid, coupon, autorenew) {
    $.ajax({
        'url': '/user/payment/purchase?payment=card',
        'data': {'shopid': shopid, 'price': amount, 'type': type, 'coupon': coupon, 'autorenew': autorenew},
        'type': 'POST',
        'success': function (data) {
            if (data.errcode == 0) {
                clientSecret = data.client_secret;
                $('#paymentID').text(data.pid);
                $('#orderID').text(data.oid);
                $('#paymentAmount').text(data.amount);
                $('#paymentCreateTime').text(data.created);
                $('#stripe-card-modal').modal({
                    'backdrop': 'static',
                    'keyboard': false
                });
                $('#stripe-card-modal').modal('show');
            } else {
                Swal.fire('发生错误', data.errmsg, 'error');
            }
        }
    });
}
...
function selectItem(method, value) {
    if (!sites.includes(location.host)) {
        return false;
    }
    ;$('#' + method + '-selection .active').removeClass('active');
...
function topUp(amount, type, shopid, coupon, autorenew) {
    if (!sites.includes(location.host)) {
        return false;
    }
    ;
    ...
    if (paymentSystem == 'f2fpay_payjs') {
        f2fpay_payjs(type, amount);
    }
    if (paymentSystem == 'stripe') {
        stripe(type, amount, shopid, coupon, autorenew);
    }
    if (paymentSystem == 'payssion') {
        payssion(type, amount, shopid, coupon, autorenew);
    }
    if (paymentSystem == 'yftpay') {
        yft(amount);
    }
    if (paymentSystem == 'malio') {
        malioPay(type, amount);
    }
    if (paymentSystem == 'banana') {
        switch (type) {
            case 'alipay_payssion':
                banana_payssion('alipay', amount, shopid, coupon, autorenew);
                break;
            case 'alipay_stripe':
                banana_stripe('alipay', amount, shopid, coupon, autorenew);
                break;
            case 'wechat':
                banana_stripe(type, amount, shopid, coupon, autorenew);
                break;
            case 'card':
                createCardPaymentIntent(type, amount, shopid, coupon, autorenew)
                break;
            default:
                Swal.fire('错误', '充值方式选择错误？', 'error');
        }
    }

    if (type != 'card') {
        tid = setTimeout(_0x2e5175, 100000);
    }
        ...
        $.ajax({
            ...
            'success': function (_0x50f5d6) {
                if (_0x50f5d6['ret']) {
                    console.log(confirmShop);
                    swal({
                        'type': 'success', 'title': '是否购买成功以支付界面为准', 'onClose': () => {
                            window.location.assign('/user');
                        }
                    });
                }
    ...
    $.ajax({
        ...
        'success': function (res) {
            if (res.ret) {
                ...
                var type = $('#payment-selection .active').attr('id');
                ...
                if (_0x21cd40 <= 0) {
                    ...
                    topUp(_0x21cd40, type, shop.id, coupon.code, confirmShop.autorenew);
                }
